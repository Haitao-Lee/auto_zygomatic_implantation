...
def transform_obb_box(center, normal, distance=screw_setting.crop_radius):
    cylinder_actor = visualization.get_screw_cylinder_actor(center, normal, distance, 6*distance, 12)
    cylinder_actor.GetProperty().SetColor(1,1,1)
    cylinder_actor.GetProperty().SetOpacity(0.2)
    polydata = visualization.get_screw_cylinder_polydata(center, normal, distance, 6*distance, 12)
...
def save_polydata_to_stl(polydata, filename):
    # 创建一个写入器
